  ...
  async handleAttendanceAndSummaryDirect() {
    ...
    for (const user of users) {
      ...
      if (logs.length === 0) {
        ...
        if (new Date(logs[0].logDate).toDateString() === today) {
          status = 'Present';
          subStatus = 'Single punch (today)';
        } else {
          status = 'Missed Punch';
        ...
        if (diffInMs < 1) {
          ...
          // ✅ Special handling if it's today
          if (new Date(firstLog).toDateString() === today) {
            status = 'Present';
            subStatus = 'Ongoing work day';
          } else {
            if (diffInHours >= 8.5) {
              status = 'Full Day';
          ...
          if (punchInTime > latePunchInThreshold) {
            ...
            latePunchBy = Math.floor(diffMs / 60000); // minutes
          }
          ...
          if (punchOutTime < earlyExitThreshold && punchOutTime.toDateString() !== today) {
            // ✅ Don't mark early exit for today since work may still be ongoing
            const diffMs = earlyExitThreshold.getTime() - punchOutTime.getTime();
            earlyExitBy = Math.floor(diffMs / 60000);
          }
        }
      }
